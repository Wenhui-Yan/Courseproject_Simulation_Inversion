...
for t=1:1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    LAI		=	20.;     	% leaf area index (m^2/m^2)
    hspot	=	0.1;       % hot spot
    tts		=	34.5;		% solar zenith angle (?
    ...
    [rdot,rsot,rddt,rsdt]=PRO4SAIL_needlespec(N,Cab,Car,Cbrown,Cw,Cm,LIDFa,LIDFb,TypeLidf,LAI,hspot,tts,tto,psi,rsoil0);
...
plot(data(:,1),data(:,2),'*');
xlabel('Canopy reflectance ');
ylabel('Leaf reflectance ');
axis([0,1,0,0.5]);
title('Transfer reflectance from canopy to leaf scale');
grid on
% % plot(data(:,1),data(:,2),'*');
