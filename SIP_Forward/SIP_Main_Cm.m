% ********************************************************************************
%% Plot of Reflectance and Transmittance with varying Cm (SIP-Leaf Model)
%% Author: WenhuiYan
%% Affiliation: CAU
...
%% ---- Parameter range ----
Cm_all = 0.002:0.001:0.012;     % Dry matter content (g/cm^2)
nC = length(Cm_all);
...
for i = 1:nC
    ...
    % ---- Color mapping ----
    idx = round( (Cm - min(Cm_all)) / (max(Cm_all) - min(Cm_all)) * (nColor-1) ) + 1;
    col = cmap(idx,:);
...
%% ---- Title ----
title('$\mathrm{Reflectance\ and\ Transmittance\ with\ varying\ } C_{m}\ \mathrm{(SIP\!-\!Leaf)}$', ...
...
c.TickLabels = compose('%.3f', Cm_all);
c.Label.String = '$C_{m}\ (\mathrm{g/cm^2})$';
c.Label.Interpreter = 'latex';
set(c, 'FontName', 'Times New Roman', 'FontSize', 9);

%% ---- Save ----
filename = 'Cm_SIP_Leaf.png';
